...
export default function AdminDashboardPage() {
  ...
  const kpis = [
    { title: "Total Bookings", value: metrics?.totalBookings ?? 32, icon: CalendarDays, change: "+12%", color: "border-[#23b5d3]/40" },
    { title: "Total Users", value: metrics?.totalUsers ?? totalUsersCount, icon: Users, change: "+15%", color: "border-[#23b5d3]/40" },
    { title: "Total Salons", value: metrics?.totalSalons ?? totalSalonsCount, icon: Store, change: "+4 Active", color: "border-[#23b5d3]/40" },
    { title: "Completed Today", value: metrics?.completedToday ?? 21, icon: CheckCircle2, change: "92% rate", color: "border-emerald-400" },
    { title: "Total Revenue", value: metrics ? `₹${metrics.totalRevenue.toLocaleString()}` : "₹12,500", icon: IndianRupee, change: "+18%", color: "border-[#23b5d3]/40" },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-[#cce0e6]">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-[#0c242c] flex items-center gap-2.5">
            ...
            <span className="px-2.5 py-0.5 text-xs font-extrabold rounded-full bg-[#23b5d3] text-white shadow-xs">
              Java API Connected
            </span>
          </h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 text-xs font-extrabold bg-white hover:bg-[#EFF5F7] text-[#0c242c] rounded-xl border border-[#c2dee6] transition-all duration-150 shadow-xs"
          >
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between text-[#0c242c]/70">
                ...
                <Icon className="w-4 h-4 text-[#23b5d3]" />
              </div>
              <div className="flex items-baseline justify-between">
                <span className="text-2xl font-extrabold text-[#0c242c] tracking-tight">{kpi.value}</span>
                <span className="text-[10px] font-extrabold px-1.5 py-0.5 rounded bg-[#23b5d3]/15 text-[#147a90] border border-[#23b5d3]/30">
                  {kpi.change}
                </span>
              </div>
            </div>
      ...
      <div className="p-6 rounded-2xl bg-white border border-[#c2dee6] space-y-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#EFF5F7]">
          <div>
            <h2 className="text-lg font-extrabold text-[#0c242c] flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-[#23b5d3]" />
              Revenue & Appointments Growth Graph
            </h2>
            <p className="text-xs text-[#0c242c]/70 font-medium mt-0.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="px-3 py-1.5 text-xs font-bold bg-[#EFF5F7] border border-[#c2dee6] rounded-xl text-[#0c242c] focus:outline-none focus:ring-2 focus:ring-[#23b5d3]"
            >
        ...
        <div className="space-y-3 pt-2">
          <div className="h-64 w-full flex items-end justify-between gap-3 sm:gap-6 px-2 sm:px-6 pt-6 pb-2 bg-[#EFF5F7] rounded-xl border border-[#c2dee6]">
            {chartData.map((item, idx) => (
              <div key={idx} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group relative">
                
                {/* Tooltip on Hover */}
                <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-[#0c242c] text-white text-[10px] font-bold px-2 py-1 rounded-lg shadow-xl pointer-events-none whitespace-nowrap z-20">
                  {item.day}: ₹{item.revenue} ({item.bookings} Bookings)
                </div>

                {/* Bar */}
                <div className="w-full max-w-[40px] bg-gradient-to-t from-[#1a9fba] to-[#23b5d3] rounded-t-lg transition-all duration-500 group-hover:brightness-110 shadow-sm relative" style={{ height: item.height }}>
                  <div className="absolute inset-x-0 top-0 h-1 bg-white/40 rounded-t-lg" />
                </div>

                {/* Label */}
                <span className="text-xs font-extrabold text-[#0c242c]">{item.day}</span>
              </div>
            ))}
          </div>

          <div className="flex items-center justify-between text-xs text-[#0c242c]/70 font-semibold px-2">
            <span className="flex items-center gap-2">
              <span className="w-3 h-3 rounded bg-[#23b5d3]" /> Revenue (₹) & Booking Volume
            </span>
      ...
      <div className="p-5 rounded-2xl bg-white border border-[#c2dee6] space-y-4 shadow-xs">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CalendarDays className="w-5 h-5 text-[#23b5d3]" />
            <h2 className="text-base font-extrabold text-[#0c242c]">Recent Salon Appointments</h2>
          </div>
          <Link
            href="/admin/appointments"
            className="text-xs font-extrabold text-[#23b5d3] hover:underline"
          >
            View All →
          </Link>
        </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#cce0e6]/60">
              {displayQueue.map((item) => (
                <tr key={item.id} className="hover:bg-[#EFF5F7]/80 transition-colors">
                  <td className="px-3 py-3 font-extrabold text-[#23b5d3]">#{item.id}</td>
                  <td className="px-3 py-3 font-bold text-[#0c242c]">{item.customerName}</td>
                  <td className="px-3 py-3 text-[#0c242c]/80 font-medium">{item.serviceName}</td>
                  <td className="px-3 py-3 text-[#0c242c]/70 font-medium">{item.staffName}</td>
                  <td className="px-3 py-3 font-mono font-bold text-[#0c242c]">{item.appointmentTime}</td>
                  <td className="px-3 py-3 text-right">
                    <span className={`px-2 py-0.5 text-[10px] font-extrabold rounded-full border ${
                      item.status === "SERVING" 
                        ? "bg-[#23b5d3] text-white border-[#23b5d3] animate-pulse" 
